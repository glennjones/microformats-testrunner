...
function test(request, reply) {
	var options = getPathObject(request);
	var filePath = testPath.getLocalPath( options, config );	
	...
	testCollection.getTest( filePath, options, function(err, test){
...
function testHTML(request, reply) {
	var options = getPathObject(request),
		filePath = testPath.getLocalPath( options, config );
...
function textcontent(request, reply) {
	reply.view('textcontent.html', {});
}
