  ...
  function getTests( pathList, callback ){
    var out = [],
        count = 0;
        
    pathList.forEach(function(path){
        getTest( path, function(err,data){
          if(!err){
            out.push(data);
          }
          count++;
  ...
  // get a test object
  function getTest( filePath, callback ){
  ...
  function getTestsGroupedByVersion( 	dirPath, versions, callback ){
      var count = 0,
          out = [];
      
      versions.forEach(function(version){
          getTestList(dirPath, version.name, function(err, fileList){
            getTests( fileList, function(err, data){
